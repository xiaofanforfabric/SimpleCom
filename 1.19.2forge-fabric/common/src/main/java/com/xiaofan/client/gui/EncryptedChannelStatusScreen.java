...
public class EncryptedChannelStatusScreen extends Screen {
    ...
    protected void init() {
        super.init();
        int centerX = width / 2;
        addDrawableChild(new ButtonWidget(centerX - 100, height / 2 + 10, 200, 20, Text.literal(EXIT), b -> onExit()));
    }
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (name == null) name = "";
        drawCenteredText(matrices, textRenderer, Text.literal(String.format(CURRENT, name)), width / 2, height / 2 - 25, 0xA0A0A0);
