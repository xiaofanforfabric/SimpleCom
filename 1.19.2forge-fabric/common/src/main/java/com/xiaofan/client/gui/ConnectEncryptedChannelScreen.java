...
public class ConnectEncryptedChannelScreen extends Screen {
    ...
    protected void init() {
        ...
        for (int i = 0; i < NAMES_PER_PAGE; i++) {
            final int index = i;
            ButtonWidget btn = new ButtonWidget(centerX - btnWidth / 2, startY + i * 22, btnWidth, btnHeight, Text.literal("..."), b -> {
                int idx = page * NAMES_PER_PAGE + index;
                if (idx >= 0 && idx < names.size() && client != null) {
                    client.setScreen(new JoinEncryptedChannelScreen(parent, names.get(idx)));
                }
            });
            btn.visible = false;
            nameButtons[i] = btn;
        ...
        addDrawableChild(new ButtonWidget(centerX - 105, height / 2 + 65, 100, 20, Text.literal(PREV), b -> {
            if (page > 0) {
                page--;
                refreshNameButtons();
            }
        }));
        addDrawableChild(new ButtonWidget(centerX + 5, height / 2 + 65, 100, 20, Text.literal(NEXT), b -> {
    ...
    private void refreshNameButtons() {
        ...
        for (int i = 0; i < NAMES_PER_PAGE; i++) {
            ...
            btn.y = startY + i * 22;
            btn.setMessage(Text.literal(idx < names.size() ? names.get(idx) : ""));
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (names.isEmpty() && page == 0) {
            drawCenteredText(matrices, textRenderer, Text.literal("正在获取列表..."), width / 2, height / 2 - 30, 0xA0A0A0);
        }
