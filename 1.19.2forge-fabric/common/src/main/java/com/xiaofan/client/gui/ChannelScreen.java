...
public class ChannelScreen extends Screen {
    ...
    protected void init() {
        ...
        channelField = new TextFieldWidget(textRenderer, centerX - fieldWidth / 2, height / 2 - 30, fieldWidth, fieldHeight, Text.literal("channel"));
        ...
        addDrawableChild(new ButtonWidget(centerX - 105, height / 2 + 10, 100, 20, Text.literal(CANCEL), b -> onClose()));
        addDrawableChild(new ButtonWidget(centerX + 5, height / 2 + 10, 100, 20, Text.literal(CONFIRM), b -> onConfirm()));

        addDrawableChild(new ButtonWidget(centerX - 105, height / 2 + 40, 100, 20, Text.literal(BTN_CREATE_ENCRYPTED), b -> {
            if (client != null) client.setScreen(new CreateEncryptedChannelScreen(parent));
        }));
        addDrawableChild(new ButtonWidget(centerX + 5, height / 2 + 40, 100, 20, Text.literal(BTN_CONNECT_ENCRYPTED), b -> {
            if (client != null) client.setScreen(new ConnectEncryptedChannelScreen(parent));
        }));
    }
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        drawCenteredText(matrices, textRenderer, Text.literal(HINT), width / 2, height / 2 - 45, 0xA0A0A0);
