...
public class JoinEncryptedChannelScreen extends Screen {
    ...
    public JoinEncryptedChannelScreen(Screen parent, String channelName) {
        super(Text.literal("加入加密信道: " + (channelName != null ? channelName : "")));
        this.parent = parent;
    ...
    protected void init() {
        ...
        passwordField = new TextFieldWidget(textRenderer, centerX - fieldWidth / 2, height / 2 - 20, fieldWidth, fieldHeight, Text.literal("password"));
        passwordField.setMaxLength(64);
        addSelectableChild(passwordField);

        addDrawableChild(new ButtonWidget(centerX - 105, height / 2 + 15, 100, 20, Text.literal(BACK), b -> closeToParent()));
        ...
        client.execute(() -> {
            if (result.success) {
                ...
                if (client.player != null) {
                    client.player.sendMessage(Text.literal("已加入加密信道：" + channelName + " (ID " + result.channelId + ")"), false);
                }
                closeToParent();
            } else {
                if (client.player != null) {
                    client.player.sendMessage(Text.literal("§c加入失败，密码错误或信道不存在"), false);
                }
            }
        });
    }
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        drawCenteredText(matrices, textRenderer, Text.literal("加入 " + channelName + " 信道，请输入密码"), width / 2, height / 2 - 40, 0xA0A0A0);
