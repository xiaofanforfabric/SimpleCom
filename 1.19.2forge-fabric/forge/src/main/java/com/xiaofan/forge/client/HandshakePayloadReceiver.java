...
public final class HandshakePayloadReceiver {
    ...
    public static void register() {
        ...
        KeyMappingRegistry.register(SimpleComVoiceClient.VOICE_KEY);
        KeyMappingRegistry.register(SimpleComVoiceClient.CHANNEL_KEY);
        ...
        SimpleComVoiceClient.setActionBarCallback(text -> {
            ...
            if (p != null) p.sendMessage(Text.literal(text), true);
        });
        ...
        VOICE_CHANNEL.addListener(event -> {
            ...
            if (buf == null) return;
            net.minecraftforge.network.NetworkEvent.Context ctx = event.getSource() != null ? event.getSource().get() : null;
            if (ctx == null) return;
        ...
        CHANNEL_SWITCH_ACK_CHANNEL.addListener(event -> {
            ...
            ctx.enqueueWork(() -> {
                ClientPlayerEntity p = MinecraftClient.getInstance().player;
                if (p != null) {
                    p.sendMessage(Text.literal("信道已成功切换到" + finalCh), false);
                }
            });
            ctx.setPacketHandled(true);
        });
        ...
        public static void handle(HandshakeMessage msg, Supplier<NetworkEvent.Context> ctx) {
            ...
            context.enqueueWork(() -> {
                try {
                    ...
                    if (player != null) {
                        player.sendMessage(Text.literal(SimpleComVoiceClient.getConnectSuccessMessage(msg.serverType)), false);
                        player.sendMessage(Text.literal(SimpleComVoiceClient.getCompressionStatusMessage(msg.useCompressionEncoder)), false);
                        player.sendMessage(Text.literal(SimpleComVoiceClient.getLowLatencyStatusMessage(msg.lowLatency)), false);
        ...
        handshakeTimeoutTask = scheduler.schedule(() -> {
            ...
            client.execute(() -> {
                if (!HandshakePayloadAdapter.hasServerPlugin() && client.player != null) {
                    client.player.sendMessage(Text.literal(MSG_TIMEOUT), false);
                }
            });
