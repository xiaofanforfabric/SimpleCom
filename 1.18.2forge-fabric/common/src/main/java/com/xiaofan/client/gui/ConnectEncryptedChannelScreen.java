...
public class ConnectEncryptedChannelScreen extends Screen {
    ...
    protected void init() {
        ...
        addDrawableChild(new ButtonWidget(centerX - 105, height / 2 + 65, 100, 20, new LiteralText(PREV), b -> {
            if (page > 0) {
                page--;
                refreshNameButtons();
            }
        }));
        addDrawableChild(new ButtonWidget(centerX + 5, height / 2 + 65, 100, 20, new LiteralText(NEXT), b -> {
