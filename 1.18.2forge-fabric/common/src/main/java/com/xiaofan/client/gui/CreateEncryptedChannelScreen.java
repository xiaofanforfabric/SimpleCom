...
public class CreateEncryptedChannelScreen extends Screen {
    ...
    protected void init() {
        ...
        nameField = new TextFieldWidget(textRenderer, centerX - fieldWidth / 2, height / 2 - 55, fieldWidth, fieldHeight, new LiteralText(NAME_HINT));
        nameField.setMaxLength(32);
        addSelectableChild(nameField);

        passwordField = new TextFieldWidget(textRenderer, centerX - fieldWidth / 2, height / 2 - 25, fieldWidth, fieldHeight, new LiteralText(PWD_HINT));
        passwordField.setMaxLength(64);
        addSelectableChild(passwordField);

        addDrawableChild(new ButtonWidget(centerX - 105, height / 2 + 15, 100, 20, new LiteralText(CANCEL), b -> closeToParent()));
        addDrawableChild(new ButtonWidget(centerX + 5, height / 2 + 15, 100, 20, new LiteralText(CONFIRM), b -> onConfirm()));
        ...
        client.execute(() -> {
            if (result.success) {
                ...
                if (client.player != null) {
                    client.player.sendMessage(new LiteralText("已创建并加入加密信道：" + nameField.getText().trim() + " (ID " + result.channelId + ")"), false);
                }
                closeToParent();
            } else {
                if (client.player != null) {
                    client.player.sendMessage(new LiteralText("§c创建失败，信道名称可能已存在"), false);
                }
            }
        });
    }
    ...
    private void onConfirm() {
        ...
        if (name.isEmpty()) {
            if (client != null && client.player != null) {
                client.player.sendMessage(new LiteralText("§c请输入信道名称"), false);
            }
            return;
        }
