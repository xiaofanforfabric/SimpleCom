...
public final class ServerPayloadHandlerFabric extends ServerPayloadHandler {
    ...
    public ServerPayloadHandlerFabric(MinecraftServer server) {
        ...
        this.config = SimpleComServerConfig.load(configDir, "fabric");
        setEncryptedChannelStore(new EncryptedChannelStore(configDir));
    ...
    public void register() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(SimpleComChannels.ENCRYPTED_CREATE, (srv, player, h, buf, resp) -> {
            byte[] data = new byte[buf.readableBytes()];
            buf.readBytes(data);
            onEncryptedCreate(player.getUuid(), data);
        });
        ServerPlayNetworking.registerGlobalReceiver(SimpleComChannels.ENCRYPTED_LIST, (srv, player, h, buf, resp) -> onEncryptedList(player.getUuid()));
        ServerPlayNetworking.registerGlobalReceiver(SimpleComChannels.ENCRYPTED_JOIN, (srv, player, h, buf, resp) -> {
            byte[] data = new byte[buf.readableBytes()];
            buf.readBytes(data);
